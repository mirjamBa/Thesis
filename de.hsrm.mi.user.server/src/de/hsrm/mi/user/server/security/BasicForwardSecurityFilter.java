...
public class BasicForwardSecurityFilter extends AbstractChainableSecurityFilter {
	private IScoutLogger logger = ScoutLogManager
			.getLogger(BasicForwardSecurityFilter.class);
	...
	public void init(FilterConfig config) throws ServletException {
		...
		if (url == null) {
			throw new IllegalArgumentException("Missing config parameter '"
					+ AUTH_SERVLET_URL_PARAM + "'");
		} else {
			try {
				m_url = new URL(url);
			} catch (MalformedURLException e) {
				throw new ServletException(
						"unable to parse authentication servlet", e);
			}
		}
	}

	@Override
	protected int negotiate(HttpServletRequest req, HttpServletResponse resp,
			PrincipalHolder holder) throws IOException, ServletException {
		String h = req.getHeader(AUTH_HEADER);
		if (h != null && h.startsWith(AUTH_HEADER_PREFIX)) {
			String[] a = new String(Base64Utility.decode(h
					.substring(AUTH_HEADER_PREFIX.length())), "ISO-8859-1")
					.split(":", 2);
			String user = a[0];
			String pass = a[1];
			logger.info("Login try: " + user + pass);
	...
	protected boolean validateUser(String user, String pass)
			throws ServletException {
		logger.info("validate user");
		try {
			// forward user name and password to the server. there it will be
			// checked whether the combination is correct or not
