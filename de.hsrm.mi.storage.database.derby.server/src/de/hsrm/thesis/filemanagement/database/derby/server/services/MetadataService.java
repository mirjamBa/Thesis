...
public class MetadataService extends AbstractService implements IMetadataService {
	...
	private boolean existsMetadataForFile(Long attributeId, Long fileId) throws ProcessingException {
		...
		SQL.selectInto(
				...
				new NVPair("fileId", fileId), new NVPair("attributeId", attributeId), new NVPair("ih", ih));
		return ih.getValue() > 0;
	}

	@Override
	public void addMetadata(Long attributeId, Long fileId, String metadata) throws ProcessingException {
		SQL.insert("INSERT INTO metadata (file_id, attribute_id, value) VALUES (:fileId, :attrId, :value)", new NVPair(
				"fileId", fileId), new NVPair("attrId", attributeId), new NVPair("value", metadata));
	}

	@Override
	public Object[][] getMetadata(Long fileId) throws ProcessingException {
		return SQL.select("SELECT M.ATTRIBUTE_ID, " + "                   A.NAME, " + "                   M.VALUE, "
				+ "                   A.DATATYPE " + "            FROM   METADATA M, "
				+ "                   METADATA_ATTRIBUTE A " + "           WHERE   M.FILE_ID = :fileId "
				+ "           AND     M.ATTRIBUTE_ID = A.ATTRIBUTE_ID " + "           ORDER BY M.ATTRIBUTE_ID ");
	}
	...
	public Object[][] getMetadataAttributeMatrix(Long fileId, Long filetypeId, boolean onlyAttributesForFiletyp)
		...
		String query = "SELECT A.ATTRIBUTE_ID, " + "            A.NAME, " + "            M.VALUE, "
				+ "            A.DATATYPE " + "    FROM    METADATA_ATTRIBUTE A " + "    LEFT OUTER JOIN METADATA M "
				+ "    ON      A.ATTRIBUTE_ID = M.ATTRIBUTE_ID " + "    WHERE   1 = 1 "
	...
	public Object getMetadataValue(String attributeName, Long fileId) throws ProcessingException {
		...
		SQL.selectInto("SELECT value FROM metadata WHERE attribute_id = :attrId AND file_id = :fileId INTO :value",
				new NVPair("attrId", attributeId), new NVPair("fileId", fileId), new NVPair("value", value));
	...
	public void updateOrInsertMetadata(Long attributeId, Long fileId, String metadata) throws ProcessingException {
		if (existsMetadataForFile(attributeId, fileId)) {
			SQL.update("UPDATE metadata SET value = :value WHERE file_id = :fileId AND attribute_id = :attributeId",
					new NVPair("fileId", fileId), new NVPair("attributeId", attributeId), new NVPair("value", metadata));
		} else {
