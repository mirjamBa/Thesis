...
import de.hsrm.perfunctio.core.shared.security.CreateAttributePermission;
import de.hsrm.perfunctio.core.shared.security.DeleteAttributePermission;
import de.hsrm.perfunctio.core.shared.security.ReadAttributePermission;
import de.hsrm.perfunctio.core.shared.security.UpdateAttributePermission;
...
public class AttributeService extends AbstractService
	...
	public Object[][] getAttributeData(String attributenName)
		...
		return SQL.select("SELECT attribute_id, " + "					name, "
				+ "					datatype " + "			FROM metadata_attribute "
	...
	@Override
	public Object[][] getAttributes(Long filetypeId, boolean addGeneralAttr)
			throws ProcessingException {
		if (!ACCESS.check(new ReadAttributePermission())) {
			throw new VetoException(TEXTS.get("VETOReadAttributePermission"));
		}
		...
		if (filetypeId != null && !addGeneralAttr) {
			s.append("            WHERE filetype_id = :filetypeId ");
		} else if (filetypeId != null && addGeneralAttr) {
			s.append("            WHERE (filetype_id = :filetypeId OR filetype_id IS NULL)");
		}
