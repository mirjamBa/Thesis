...
public class FileService extends AbstractService implements IFileService {
	...
	public FileFormData create(FileFormData formData, ServerFileData fileData,
		...
		// start handler (fileId not available)
		de.hsrm.perfunctio.core.server.Activator.getDefault().handle(null,
				formData, fileData, parentFolderId);
		
		//create server file
		SERVICES.getService(IFileStorageService.class).saveFile(fileData);
	...
	public boolean deleteFile(Long fileId) throws ProcessingException {
		...
		return SERVICES.getService(IFileStorageService.class).deleteServerFile(
				path);
	}
	...
	private Object[][] getFileTableData(FileSearchFormData searchFormData,
		...
		if (searchFormData.getGeneralSearch().getValue() != null) {
			stringBuilder
					.append(" AND EXISTS (SELECT 1 FROM METADATA MD "
		...
		for (int i = 0; i < rows; i++) {
			...
			if (value != null) {
				...
				stringBuilder.append(" AND EXISTS (SELECT 1 FROM METADATA MD "
						...
						+ "					AND UPPER(MD.VALUE) LIKE UPPER('%' || :value"
						+ i + " || '%') "
						+ "					AND MD.FILE_ID = F.FILE_FOLDER_ID) ");
		...
		if (searchFormData.getTag().getValue() != null) {
			Long[] tagIds = searchFormData.getTag().getValue();

			if (searchFormData.getAndOr().getValue()) {
				// AND linkage
				for (int i = 0; i < tagIds.length; i++) {
					Long tagId = tagIds[i];
					stringBuilder
							.append(" AND EXISTS( SELECT 1 FROM TAG_FILE T "
									+ "					WHERE T.TAG_ID = :tagId"
									+ i
									+ " "
									+ "					AND   T.FILE_ID = F.FILE_FOLDER_ID) ");
					bindings.put("tagId" + i, tagId);

				}
			} else {
				// OR linkage
				stringBuilder.append(" AND EXISTS( SELECT 1 FROM TAG_FILE T "
						+ "					WHERE T.TAG_ID = :tag "
						+ "					AND   T.FILE_ID = F.FILE_FOLDER_ID) ");
			}
		}
		...
		if (maxRowCount != null && maxRowCount > 0) {
			...
			stringBuilder.append(" ORDER BY F.FILE_FOLDER_ID " + "				OFFSET "
					+ minRow + " ROWS " + "				FETCH NEXT "
					+ maxRowCount.intValue() + " ROWS ONLY ");
	...
	public int getNumberOfFiles(Long filetype) throws ProcessingException {
		...
		SQL.selectInto(
				"SELECT distinct count(f.file_folder_id) "
						+ "		FROM file_folder f,"
						+ "			 metadata m "
						...
						+ "                     (SELECT R.ROLE_ID FROM USER_ROLE R WHERE R.U_ID = :userId) "
						+ "             	) " + "             ) "
						+ "		INTO :number ", new NVPair("attr", attrId),
				new NVPair("value", filetype), new NVPair("number", number),
